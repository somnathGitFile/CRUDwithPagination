from django.shortcuts import render
from.models import Customer
from django.views.generic import ListView,CreateView,UpdateView,DeleteView
from django.urls import reverse_lazy

# Create your views here.
class ShowCustomer(ListView):
    model=Customer
    paginate_by=3

class AddCustomer(CreateView):
    model=Customer
    fields='__all__'
    success_url=reverse_lazy('showCust_url')

class UpdateCustomer(UpdateView):
    model=Customer
    fields='__all__'
    success_url=reverse_lazy('showCust_url')

class DeleteCustomer(DeleteView):
    model=Customer
    fields='__all__'
    success_url=reverse_lazy('showCust_url')


from django.http import FileResponse
from fpdf import FPDF


def GenaratePDF(request):

    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    pdf.set_font('courier', 'B', 16)
    #pdf.cell(40, 10, 'This is what you have sold this month so far:',0,1)
   # pdf.cell(40, 10, '',0,1)
    pdf.set_font('courier', '', 12)
   # pdf.cell(200, 8, f"{'Item'.ljust(30)} {'Amount'.rjust(20)}", 0, 1)
    #pdf.line(10, 30, 150, 30)
    #pdf.line(10, 38, 150, 38)
    lines=[]

    farm = Customer.objects.all()

    for f in farm:
         lines.append(f.customer_id)
         lines.append(f.customer_name)
         lines.append(f.customer_email)
         lines.append(f.customer_address)
         lines.append(f.customer_pan)
         lines.append(f.customer_adhar)
         lines.append(" ")

    for line in lines:
        pdf.cell(200, 8,f"{line}", 0, 1)
    pdf.output('Customer.pdf', 'F')
    return FileResponse(open('Customer.pdf', 'rb'), as_attachment=True, content_type='application/pdf')

    

def perticularData(request,pk):

    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    pdf.set_font('courier', 'B', 16)
    pdf.cell(40, 10, 'This is what you have sold this month so far:',0,1)
    pdf.cell(40, 10, '',0,1)
    pdf.set_font('courier', '', 12)
   # pdf.cell(200, 8, f"{'Item'.ljust(30)} {'Amount'.rjust(20)}", 0, 1)
   # pdf.line(10, 30, 150, 30)
    #pdf.line(10, 38, 150, 38)
    

    farm = Customer.objects.get(id=pk)
    DI=farm.customer_id
    NM=farm.customer_name
    ML=farm.customer_email
    AD=farm.customer_address
    PN=farm.customer_pan
    ADR=farm.customer_adhar
    lines=[DI,NM,ML,AD,PN,ADR]
    

    for line in lines:
        pdf.cell(200, 8,f"{line}", 0, 1)
    pdf.output(f'{NM}.pdf', 'F')
    return FileResponse(open(f'{NM}.pdf', 'rb'), as_attachment=True, content_type='application/pdf')



# from django.http import FileResponse
# from reportlab.pdfgen import canvas
# import io
# from reportlab.lib.units import inch
# from reportlab.lib.pagesizes import letter

# def GenaratePDF(request):
#     buf = io.BytesIO()
#     c= canvas.Canvas(buf, pagesize=letter, bottomup=0)
#     textob = c.beginText()
#     textob.setTextOrigin(inch,inch)
#     textob.setFont('Helvetica', 14)
#     lines =[]
#     farm = Customer.objects.all()

#     for f in farm:
#         lines.append(f.customer_id)
#         lines.append(f.customer_name)
#         lines.append(f.customer_email)
#         lines.append(f.customer_address)
#         lines.append(f.customer_pan)
#         lines.append(f.customer_adhar)
#         lines.append(" ")

#     for line in lines:
#         textob.textLine(line)

#     c.drawText(textob)
#     c.showPage()
#     c.save()
#     buf.seek(0)

#     return FileResponse(buf, as_attachment=True, filename='xyz.pdf')
